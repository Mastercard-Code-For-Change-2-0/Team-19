...
const DashboardAdmin = () => {
  ...
  const handleMatchApproval = async (matchId, status) => {
    setIsLoading(true);
    try {
  ...
  const handleCreateMatch = async (donationId, requestId) => {
    setIsLoading(true);
    try {
  ...
  const getMatchedDonation = (matchId) => {
    const match = matches.find(m => m.id === matchId);
    return match ? donations.find(d => d.id === match.donationId) : null;
  };

  const getMatchedRequest = (matchId) => {
    const match = matches.find(m => m.id === matchId);
